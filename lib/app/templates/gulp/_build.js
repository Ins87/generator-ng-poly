...
module.exports = function (gulp, $, config) {
  ...
  gulp.task('markup', function () {
    return gulp.src([
      ...
      .pipe($.cached('markup'))<% if (markup === 'haml') { %>
  ...
  gulp.task('bowerAssets', function () {
    ...
    return gulp.src($.mainBowerFiles(), {base: bowerDir})
      .pipe(assetFilter)
      .pipe($.cached('bowerAssets', {optimizeMemory: true} ))
      .pipe(gulp.dest(config.extDir))
  ...
  gulp.task('copyTemplates', [<% if (polymer) { %>'componentsInject'<% } else { %>'markup'<% } %>], function () {
    // always copy templates to testBuild directory
    var stream = $.streamqueue({objectMode: true});

    stream.queue(gulp.src([config.buildDirectiveTemplateFiles])
      .pipe($.cached('copyTestDirectiveTemplates')));
